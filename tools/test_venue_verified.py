...
def main() -> None:
    ...
    owner = _opener()
    c1, html = get(owner, f"{base}/profile")
    ...
    c2 = post_form(owner, f"{base}/profile/privacy", {"privacy": "venue_verified"})
    print("set_venue_verified:", c2)

    other = _opener()
    c3, _ = get(other, f"{base}/card/{pid}?venue={venue}")
    print("other_session_card:", c3, "(expect 404)")

    c4, _ = get(owner, f"{base}/card/{pid}?venue={venue}")
    print("owner_card:", c4, "(expect 200)")

    c5 = post_form(owner, f"{base}/profile/privacy", {"privacy": "public"})
    print("restore_public:", c5)
